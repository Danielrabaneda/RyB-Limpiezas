...
if ($OutputPath) { $target = [IO.Path]::GetFullPath($OutputPath) }
$allowed = @('Connect-LimpiaGest.ps1', 'ConnectorProtocol.ps1', 'Install-LimpiaGestConnector.ps1',
  'Open-LimpiaGestConnector.ps1', 'Uninstall-LimpiaGestConnector.ps1', 'InstallerPreflight.ps1', 'InstallerTransaction.ps1', 'Instalar LimpiaGest.cmd',
...
try {
  ...
  if ('ConnectorProtocol.ps1' -notin $entries -or 'VERSION.json' -notin $entries -or
      'InstallerPreflight.ps1' -notin $entries -or 'InstallerTransaction.ps1' -notin $entries) { throw 'Incomplete protocol v2 package' }
