...
export default function TransferRequestsPanel({ onActionComplete }) {
  ...
  const getOpName = (id) =>
    operarios.find((o) => o.uid === id)?.name || "Cargando...";
  ...
  async function handleReject(id) {
    if (
      !confirm(
        "¿Rechazar este traspaso? El trabajo volverá al operario original.",
      )
    )
      return;
    setProcessingId(id);
    ...
    <div className="card shadow-lg border-0 bg-white mb-8 overflow-hidden animate-fadeIn">
      <div className="card-header bg-amber-50 border-b border-amber-100 flex items-center justify-between p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-black text-amber-900 leading-tight">
              Solicitudes de Traspaso
            </h3>
            <p className="text-xs font-bold text-amber-700 opacity-70 uppercase tracking-wider">
              Validación pendiente
            </p>
          </div>
        </div>
        <span className="badge bg-amber-500 text-white border-0 font-black px-3 py-1">
          {requests.length}
        </span>
      </div>
              ...
              {req.type === "date_change" ? (
                <div className="flex items-center gap-2">
                  <div className="flex flex-col items-center min-w-[50px]">
                    ...
                    <div className="text-[10px] font-black text-slate-500 mt-1 truncate max-w-[60px]">
                      {getOpName(req.userId).split(" ")[0]}
                    </div>
                  </div>
                  <div className="flex flex-col items-center px-2">
                    <div className="text-[10px] text-slate-400 line-through">
                      {req.oldDate?.toDate
                        ? format(req.oldDate.toDate(), "d MMM", { locale: es })
                        : ""}
                    </div>
                    <div className="text-amber-500 text-lg font-black shrink-0 leading-none">
                      {" "}
                      ↓{" "}
                    </div>
                    <div className="text-sm font-black text-blue-600">
                      {req.newDate?.toDate
                        ? format(req.newDate.toDate(), "d MMM", { locale: es })
                        : ""}
                    </div>
                  </div>
                </div>
                ...
                <>
                  <div className="flex flex-col items-center min-w-[50px]">
                    ...
                    <div className="text-[10px] font-black text-slate-500 mt-1 truncate max-w-[60px]">
                      {getOpName(req.fromUserId).split(" ")[0]}
                    </div>
                  </div>

                  <div className="text-amber-500 text-xl font-black shrink-0">
                    {" "}
                    »{" "}
                  </div>
                  ...
                  <div className="flex flex-col items-center min-w-[50px]">
                    ...
                    <div className="text-[10px] font-black text-blue-500 mt-1 truncate max-w-[60px]">
                      {getOpName(req.toUserId).split(" ")[0]}
                    </div>
                  </div>
                </>
              )}
              ...
              <div className="ml-1 sm:ml-4 flex-1">
                <div className="font-black text-slate-800 text-sm leading-tight">
                  {req.type === "single"
                    ...
                    : req.type === "day"
                      ? `Traspaso Día (${req.serviceCount} serv.)`
                      : req.type === "date_change"
                        ? "Cambio de Fecha"
                ...
                <div className="text-right">
                  <div className="font-bold text-sm">
                    {(() => {
                      ...
                      try {
                        if (req.date?.toDate)
                          return format(req.date.toDate(), "d 'de' MMMM", {
                            locale: es,
                          });
                        if (req.startDate?.toDate)
                          return `${format(req.startDate.toDate(), "d/MM")} al ${format(req.endDate.toDate(), "d/MM")}`;
