          ...
          <table className="table table-grid table-striped min-w-[600px]">
            ...
            <tbody>
              {products.map((p) => {
                ...
                const catObj =
                  CATEGORIES.find(
                    (c) => c.id === (p.category || autoCategorize(p.name)),
                  ) || CATEGORIES[4];
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td className="text-center">
                      <span
                        className={`inline-block px-2 py-1 rounded font-bold text-sm ${isLow ? "bg-red-100 text-red-700" : isWarning ? "bg-orange-100 text-orange-700" : "bg-green-100 text-green-700"}`}
                      >
                        {stock}
                      </span>
                    </td>
                    <td className="text-center font-semibold text-slate-500">
                      {minStock}
                    </td>
                    <td className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          disabled={actionLoading}
                          onClick={() =>
                            setStatsModal({ open: true, product: p })
                          }
                        >
                          📊 Stats
                        </button>
