        ...
        <div className="flex flex-wrap justify-between items-start gap-2">
          <div>
            <div className="flex items-center gap-2">
              <span
                className={`status-badge ${order.status === "pending" ? "status-pending" : "status-completed"}`}
              >
        ...
        <div className="bg-slate-50 p-2 rounded text-sm">
          <p>
            ...
            {users[order.userId]?.name ||
              users[order.userId]?.displayName ||
              "Desconocido"}
          </p>
          {order.notes && (
            <p className="mt-1 italic text-slate-500">"{order.notes}"</p>
          )}
          {order.status === "pending" && (
            <p
              className={`mt-2 text-xs font-bold ${canDeliver ? "text-green-600" : "text-red-600"}`}
            >
          ...
          <button
            ...
            onClick={() => {
              if (!canDeliver) {
                if (
                  !confirm(
                    `⚠️ STOCK INSUFICIENTE. Tienes ${currentStock} y se piden ${order.quantity}. ¿Entregar de todas formas y dejar el stock en negativo?`,
                  )
                ) {
                  return;
                }
