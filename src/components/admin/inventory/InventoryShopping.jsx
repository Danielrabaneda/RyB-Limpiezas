    ...
    <div className="shopping-section">
      <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
        <div>
          ...
          <p className="text-xs text-muted">
            Productos por debajo del stock mínimo o agregados manualmente
          </p>
        </div>
                ...
                <th className="w-10 text-center">
                  <input
                    ...
                    checked={
                      ...
                      shoppingItems.every((item) => item.checked)
                    }
                    onChange={(e) => {
              ...
              {shoppingItems.length === 0 ? (
                ...
                shoppingItems.map((item) => {
                  const catObj =
                    CATEGORIES.find(
                      (c) =>
                        c.id === (item.category || autoCategorize(item.name)),
                    ) || CATEGORIES[4];
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="text-center">
                        <span
                          className={`px-2 py-0.5 rounded font-bold text-xs ${item.currentStock <= 0 ? "bg-red-100 text-red-700" : "bg-orange-100 text-orange-700"}`}
                        >
                      ...
                      <td className="text-right">
                        <div className="flex justify-end items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setShoppingItems(
                                shoppingItems.filter((i) => i.id !== item.id),
                              )
                            }
                          >
                            ✕
      ...
      <div className="card p-4">
        <h3 className="text-sm font-bold text-slate-700 mb-2">
          ➕ Añadir Producto Adicional a la Lista
        </h3>
        <div className="flex gap-3 items-center flex-wrap">
          ...
          <p className="text-xs text-muted">
            Añade al listado productos que no estén por debajo del mínimo para
            incluirlos en el reporte o compra.
          </p>
        </div>
      </div>
    </div>
