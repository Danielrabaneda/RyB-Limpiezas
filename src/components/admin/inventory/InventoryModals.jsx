...
function ModalChartTooltip({ active, payload, unit }) {
  ...
  return (
    ...
    >
      <p style={{ color: "#94a3b8", fontSize: 10, margin: 0, fontWeight: 600 }}>
        {data.label}
      </p>
      <p
        style={{
      ...
      <p style={{ color: "#64748b", fontSize: 9, margin: 0 }}>
        {data.count} entregas
      </p>
    </div>
  );
...
function ModalOperatorTooltip({ active, payload, unit }) {
  ...
  return (
    ...
    >
      <p style={{ color: "#e2e8f0", fontSize: 11, margin: 0, fontWeight: 700 }}>
        {data.name}
      </p>
      <p
        style={{
    ...
    <>
      ...
      {showAddProduct && (
        <div className="modal-overlay" onClick={() => setShowAddProduct(false)}>
          <div
            ...
            style={{ maxWidth: "400px", width: "95vw" }}
          >
            ...
            <form onSubmit={handleStockAction}>
              <div className="modal-body">
                <p className="text-sm font-bold mb-4">
                  Producto: {stockModal.product?.name}
                </p>
                <div className="form-group">
                  <label>
                    {stockModal.type === "in"
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setStockModal({ ...stockModal, notes: e.target.value })
                    }
                  />
                </div>
              </div>
      ...
      {statsModal.open &&
        ...
        (() => {
          ...
          const chartData =
            statsPeriod === "monthly" ? stats.monthly : stats.yearly;
          const maxVal = Math.max(...chartData.map((d) => d.value), 1);

          return (
            <div
              className="modal-overlay"
              onClick={() => setStatsModal({ open: false, product: null })}
            >
              <div
                className="modal rounded-3xl shadow-xl border border-slate-100 overflow-hidden"
                onClick={(e) => e.stopPropagation()}
              ...
              >
                <div className="modal-header border-b border-slate-100 bg-slate-50/50 p-5 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 font-semibold flex items-center gap-1">
                      ...
                      <span className="font-extrabold text-slate-600">
                        {statsModal.product.currentStock || 0}{" "}
                        {statsModal.product.unit}
                      </span>
                      {statsModal.product.currentStock <=
                        statsModal.product.minStock && (
                        <span className="inline-block px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 font-extrabold text-[8px] uppercase tracking-wider ml-1">
                          Stock Bajo
                        </span>
                      )}
                    </p>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setStatsModal({ open: false, product: null })
                    }
                  >
                    ✕
                  </button>
                ...
                <div className="modal-body p-6 flex flex-col gap-5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="bg-slate-50/60 p-3 rounded-2xl border border-slate-100 flex flex-col gap-0.5">
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                        Promedio ({statsPeriod === "monthly" ? "Mes" : "Año"})
                      </div>
                      <div className="text-lg font-black text-slate-800">
                        {(statsPeriod === "monthly"
                          ? stats.avgMonthly
                  ...
                  <div>
                    ...
                    {stats.totalConsumed === 0 ? (
                      <div className="text-xs italic text-slate-400 p-8 text-center bg-slate-50 border border-dashed rounded-2xl">
                        No hay registros de consumo (entregas) para este
                        producto.
                      </div>
                    ) : (
                            ...
                            >
                              ...
                              <Bar
                                ...
                                label={{
                                  ...
                                  fill: "#475569",
                                  formatter: (val) =>
                                    `${val.toFixed(1)} ${statsModal.product.unit}`,
                                }}
                              >
                ...
                <div className="modal-footer border-t border-slate-100 bg-slate-50/50 p-4">
                  <button
                    type="button"
                    className="btn btn-secondary rounded-xl font-bold py-1.5 px-4"
                    onClick={() =>
                      setStatsModal({ open: false, product: null })
                    }
                  >
