    ...
    <div className="card mt-4">
      ...
      {activeWorkday?.currentCompanionId && (
        <div className="mb-3 p-2 bg-blue-50 border border-blue-100 rounded-lg flex items-center gap-2">
          ...
          <span className="text-[10px] text-blue-700 font-medium">
            Acompañante global configurado:{" "}
            <strong>
              {operariosMap[activeWorkday.currentCompanionId] || "..."}
            </strong>
          </span>
        </div>
      )}
      ...
      {!service.companionLogs || service.companionLogs.length === 0 ? (
        <p className="text-muted text-sm">
          No hay acompañantes en este servicio.
        </p>
      ) : (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted">
                    ...
                    {log.leftAt &&
                      ` - Salida: ${format(new Date(log.leftAt), "HH:mm")}`}
                  </div>
                </div>
                {isActive &&
                  !isCompleted &&
                  log.userId !== activeWorkday?.currentCompanionId && (
                    <button
