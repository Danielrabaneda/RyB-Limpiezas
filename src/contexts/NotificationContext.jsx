...
export function NotificationProvider({ children }) {
      ...
      async (snapshot) => {
        ...
        for (const docSnap of snapshot.docs) {
          ...
          const intervalId = setInterval(() => {
            ...
            if (document.visibilityState !== "visible") {
              console.log(
                `[Notifications] Re-alerta #${repeatCount} para: ${title}`,
              );
              playNotificationSound(true);
            }
          }, REPEAT_INTERVAL_MS);
  ...
  const triggerWorkdayStartPopups = useCallback(() => {
    const workdayStartNotif = notifications.find((n) => {
      return (
        (n.triggerEvent === "workday_start" ||
          n.triggerEvent === "app_start") &&
        !shownPopupIdsRef.current.has(n.id)
      );
    });
  ...
  const triggerWorkdayEndPopups = useCallback(() => {
    const workdayEndNotif = notifications.find((n) => {
      return (
        n.triggerEvent === "workday_end" && !shownPopupIdsRef.current.has(n.id)
      );
    });
    if (workdayEndNotif) {
