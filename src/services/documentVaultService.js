import { 
  collection, doc, addDoc, getDocs, query, where, deleteDoc, 
  serverTimestamp 
 ...
 */
export async function uploadDocument({ title, category, communityId = null, fileUrl }) {
...
export async function getDocumentsByCategory(category) {
  const q = query(collection(db, 'documentVault'), where('category', '==', category));
