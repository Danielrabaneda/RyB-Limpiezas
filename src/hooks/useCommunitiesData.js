...
export default function useCommunitiesData({
  ...
  const [form, setForm] = useState({
    ...
    exitGeofenceRadiusMeters: "100",
    entryConfirmDelaySeconds: "90",
  ...
  const openEditModal = (comm) => {
    ...
    setForm({
      ...
      geofenceRadiusMeters: String(comm.geofenceRadiusMeters || 50),
      exitGeofenceRadiusMeters: String(
        comm.exitGeofenceRadiusMeters ||
          (Number(comm.geofenceRadiusMeters) || 50) + 50,
      ),
      entryConfirmDelaySeconds: String(comm.entryConfirmDelaySeconds || 90),
      exitConfirmDelaySeconds: String(comm.exitConfirmDelaySeconds || 300),
      autoCloseOnExit: comm.autoCloseOnExit === true,
    ...
    try {
      ...
      const communityData = {
        ...
        geofenceRadiusMeters: Math.max(
          10,
          parseInt(form.geofenceRadiusMeters, 10) || 50,
        ),
        exitGeofenceRadiusMeters: Math.max(
          Math.max(10, parseInt(form.geofenceRadiusMeters, 10) || 50) + 10,
          parseInt(form.exitGeofenceRadiusMeters, 10) || 100,
        ),
        entryConfirmDelaySeconds: Math.max(
          30,
          parseInt(form.entryConfirmDelaySeconds, 10) || 90,
        ),
        exitConfirmDelaySeconds: Math.max(
          60,
          parseInt(form.exitConfirmDelaySeconds, 10) || 300,
        ),
        autoCloseOnExit: form.autoCloseOnExit === true,
