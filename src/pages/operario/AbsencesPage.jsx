...
export default function AbsencesPage() {
  ...
  const getTypeLabel = (t) => {
    switch (t) {
      ...
      case 'personal_day': return '🗓️ Asuntos Propios';
      default: return t;
    }
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'approved':
        return <span className="badge badge-success" style={{ padding: '4px 8px', borderRadius: '12px' }}>✅ Aprobada</span>;
      case 'rejected':
        return <span className="badge badge-danger" style={{ padding: '4px 8px', borderRadius: '12px' }}>❌ Rechazada</span>;
      default:
        return <span className="badge badge-warning" style={{ padding: '4px 8px', borderRadius: '12px' }}>⏳ Pendiente</span>;
    }
  };

  return (
    <div className="animate-fadeIn" style={{ paddingBottom: '30px' }}>
      <h2 style={{ fontSize: 'var(--font-xl)', fontWeight: 800, marginBottom: 'var(--space-4)' }}>🌴 Ausencias y Vacaciones</h2>
      ...
      <div className="card mb-6">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="form-group">
            <label className="form-label text-xs font-bold">Tipo de Ausencia</label>
          ...
          <div className="form-group">
            <label className="form-label text-xs font-bold">Motivo / Comentarios</label>
            <textarea 
          ...
          <div className="form-group" style={{ padding: 'var(--space-3)', background: 'var(--color-bg-light)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)' }}>
            <label className="form-label text-xs font-bold mb-1 block">
              📄 Justificante o Documentación {type === 'sick_leave' && <span className="text-danger">*</span>}
            </label>
            {docUrl ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px', background: 'var(--color-success-light)', borderRadius: 'var(--radius-sm)' }}>
                <span className="text-xs font-medium text-success">✓ Archivo adjunto cargado</span>
                <button type="button" className="btn btn-ghost btn-xs text-danger" onClick={() => setDocUrl('')} style={{ border: '1px solid var(--color-danger)', fontSize: '10px' }}>Quitar</button>
              </div>
            ) : (
              <div>
                <input 
                ...
                <label 
                  ...
                  style={{ cursor: 'pointer', display: 'block', padding: '8px' }}
                >
                  {uploading ? '⏳ Subiendo...' : '📷 Subir Justificante / Foto'}
                </label>
              </div>
            )}
            <p className="text-[10px] text-muted mt-1">Sube el parte de baja o justificante médico correspondiente.</p>
          </div>

          <button 
            type="submit" 
            className="btn btn-primary w-full" 
            disabled={actionLoading || uploading || (type === 'sick_leave' && !docUrl)}
          >
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <p className="text-muted text-sm text-center py-6">No has solicitado ausencias todavía.</p>
        ) : (
          <div className="flex flex-col gap-3">
            {absences.map(abs => {
              ...
              return (
                <div key={abs.id} className="p-3 border rounded-lg" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                  ...
                  <p className="text-xs text-muted mb-2 font-semibold">
                    📅 Del {format(start, 'dd/MM/yyyy')} al {format(end, 'dd/MM/yyyy')}
                  </p>
                  
                  {abs.reason && (
                    <p className="text-xs text-muted mb-2 bg-slate-50 p-2 rounded" style={{ fontStyle: 'italic' }}>
                      💬 "{abs.reason}"
                    </p>
                  )}
