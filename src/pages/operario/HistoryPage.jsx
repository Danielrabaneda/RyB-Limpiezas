...
export default function HistoryPage() {
  ...
  const monthTotalKm = mileageMonth.reduce(
    (acc, r) => acc + (r.totalKm || 0),
    0,
  );
  ...
  const tabStyle = (tab) => ({
    ...
    color:
      activeTab === tab ? "var(--color-primary)" : "var(--color-text-muted)",
    border: "none",
    cursor: "pointer",
  });
  ...
  return (
    <div className="animate-fadeIn">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          ...
          <button
            ...
            style={tabStyle("services")}
          >
            Lista
          </button>
          <button
            onClick={() => setActiveTab("workdays")}
            style={tabStyle("workdays")}
          >
            Horas
          </button>
          <button
            onClick={() => setActiveTab("mileage")}
            style={tabStyle("mileage")}
          >
            🚗 Km
          </button>
        </div>
      ...
      {(activeTab === "services" || activeTab === "workdays") && (
        <div
          className="flex items-center justify-between mb-4 card"
          style={{ padding: "8px var(--space-4)" }}
        >
          <button
      ...
      {activeTab === "services" && (
        <div className="mb-6 flex justify-center px-4">
          {(() => {
            const hasStartedWeek = services.some(
            ...
            return (
              <button
                ...
                onClick={() =>
                  setTransferModal({ open: true, type: "week", date: null })
                }
              >
                ...
                <div className="flex items-center justify-between mb-3 border-b pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {dayServices.length > 0 &&
                      (() => {
                        const hasStartedDay = dayServices.some(
                          (s) =>
                            s.status === "completed" ||
                ...
                {dayServices.length === 0 ? (
                  ...
                  (() => {
                    ...
                    dayServices.forEach((svc) => {
                      const isOtras =
                        svc.printColor === "#ef4444" && !svc.isGarage;
                      const hasMainService = dayServices.some(
                        (s) =>
                    ...
                    return (
                      <div className="flex flex-col gap-2">
                        {Object.entries(communityGroups).map(
                          ([groupKey, group]) => {
                            const allCompleted = group.services.every(
                              (s) => s.status === "completed",
                            );
                            const allMissed = group.services.every(
                              (s) => s.status === "missed",
                            );
                            const allFinalized = group.services.every(
                              (s) =>
                                s.status === "completed" ||
                                s.status === "missed",
                            );
                            const hasStarted = group.services.some(
                              (s) =>
                                s.status === "completed" ||
                                s.status === "missed" ||
                                s.status === "in_progress",
                            ...
                            if (group.tasks.length > 0) {
                              ...
                              const missedNames = group.services
                                .map((s) => s.taskName)
                                .filter(Boolean);
                              tasksStr =
                                missedNames.length > 0
                                  ? ` (No realizado: ${missedNames.join(", ")})`
                                  : " (No realizado)";
                            }
          ...
          {weekDays
            ...
            .map((day) => {
              ...
              const daySessions = workdays.filter((wd) => {
                const wdDate = wd.date?.toDate
                  ? wd.date.toDate()
                  : new Date(wd.date);
                const wdDateStr = new Intl.DateTimeFormat("en-CA", {
                  timeZone: "Europe/Madrid",
                }).format(wdDate);
              ...
              const isAnyActive = daySessions.some(
                (s) => s.status === "active",
              );
                          ...
                          <span className="flex flex-col gap-0.5 mt-1">
                            {daySessions.map((s, idx) => (
                              <span key={s.id} className="block">
                                Sesión {idx + 1}: {safeFormatDate(s.startTime)}{" "}
                                - {safeFormatDate(s.endTime)} (
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted mt-0.5">
                        {isManual ? (
                          ...
                          <span>
                            {dayRecord.totalTramos} tramo
                            {dayRecord.totalTramos !== 1 ? "s" : ""} calculados
                          </span>
                        ) : (
                        ...
                        {hasSuspicious && (
                          ...
                          >
                            ⚠️ {dayRecord.tramosSospechosos} sospechoso
                            {dayRecord.tramosSospechosos !== 1 ? "s" : ""}
                          </span>
                        )}
                      </div>
                      ...
                      {dayRecord.tramos &&
                        dayRecord.tramos.filter((t) => !t.mismoCentro).length >
                          0 && (
                          <div
                            className="mt-1"
                            style={{
