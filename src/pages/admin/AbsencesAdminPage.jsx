...
import { getAllAbsences, approveAbsence, rejectAbsence } from '../../services/absenceService';
...
export default function AbsencesAdminPage() {
  ...
  async function handleResolve(absenceId, action) {
    ...
    try {
      ...
      setAbsences(updatedAbsences.sort((a, b) => {
        const tA = a.createdAt?.toDate ? a.createdAt.toDate().getTime() : 0;
        const tB = b.createdAt?.toDate ? b.createdAt.toDate().getTime() : 0;
        return tB - tA;
      }));
  ...
  return (
    <div className="animate-fadeIn">
      <h2 style={{ fontSize: 'var(--font-2xl)', fontWeight: 800, marginBottom: 'var(--space-6)' }}>🌴 Panel de Ausencias y Vacaciones</h2>
        ...
        <div className="flex flex-col gap-4">
          {pendingAbsences.length === 0 ? (
            ...
            pendingAbsences.map(abs => {
              ...
              return (
                <div key={abs.id} className="card p-5 border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideIn">
                  <div style={{ flex: 1 }}>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="font-bold text-lg text-slate-800">
                        👤 {operariosMap[abs.userId] || abs.userName || 'Operario'}
                      </span>
                      <span className="badge badge-warning" style={{ transform: 'scale(0.9)' }}>Pendiente</span>
                    </div>
                    
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      <p className="text-sm font-semibold text-slate-700">
                        Tipo: <span className="font-normal">{getTypeLabel(abs.type)}</span>
                      </p>
                      <p className="text-sm font-semibold text-slate-700">
                        Fechas: <span className="font-bold text-primary">Del {format(start, 'dd/MM/yyyy')} al {format(end, 'dd/MM/yyyy')}</span>
                      </p>
                      {abs.reason && (
                        <p className="text-xs text-muted mt-2 bg-slate-50 p-3 rounded" style={{ fontStyle: 'italic', borderLeft: '3px solid var(--color-primary)' }}>
                          💬 "{abs.reason}"
                        </p>
                      )}
                      {abs.docUrl && (
                  ...
                  <div className="flex gap-2 w-full md:w-auto" style={{ alignSelf: 'stretch', display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end' }}>
                    <button 
            ...
            <table className="table">
              ...
              <tbody>
                {resolvedAbsences.map(abs => {
                  ...
                  return (
                    <tr key={abs.id}>
                      <td className="font-semibold text-sm">{operariosMap[abs.userId] || abs.userName || '—'}</td>
                      ...
                      <td className="text-sm text-muted" style={{ maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={abs.reason}>
                        {abs.reason || '—'}
                      </td>
                      <td className="text-sm">
                        {abs.docUrl ? (
                          <a href={abs.docUrl} target="_blank" rel="noopener noreferrer" className="text-primary font-bold">Ver doc</a>
                        ) : (
                          <span className="text-muted">—</span>
                        )}
                      </td>
                      <td>{getStatusBadge(abs.status)}</td>
                      <td className="text-sm text-muted">{operariosMap[abs.resolvedBy] || 'Admin'}</td>
                    </tr>
                  );
                })}
                {resolvedAbsences.length === 0 && (
                  <tr><td colSpan="8" className="text-center text-muted p-6">No hay solicitudes resueltas en el historial.</td></tr>
                )}
              </tbody>
            </table>
          </div>
