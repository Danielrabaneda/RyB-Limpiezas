...
import { getEvidenceReportsRange, deleteEvidenceReport, markEvidenceReviewed } from '../../services/evidenceService';
...
export default function EvidenceReportsPage() {
  ...
  async function loadReports() {
    ...
    try {
      ...
      const data = await getEvidenceReportsRange(new Date(startDate), new Date(endDate), filters);
      setReports(data);
    } catch (err) {
  ...
  async function handleDelete(id) {
    if (!window.confirm('¿Estás seguro de que deseas eliminar este reporte de evidencia? Esta acción no se puede deshacer.')) return;
    try {
  ...
  function getOperarioName(uid) {
    return operarios.find(o => o.uid === uid)?.name || uid?.substring(0, 8) + '...';
  }
  ...
  return (
    <>
      <div className="animate-fadeIn">
        <h2 style={{ fontSize: 'var(--font-2xl)', fontWeight: 800, marginBottom: 'var(--space-6)' }}>
          📸 Evidencias
        </h2>
        ...
        <div className="filter-bar">
          ...
          <button className="btn btn-primary" onClick={loadReports}>🔍 Filtrar</button>
        </div>
        ...
        <div className="grid grid-4 gap-4 mb-6">
          ...
          <div className="stat-card">
            <div className="stat-value" style={{ color: 'var(--color-warning)' }}>{pendingReports}</div>
            <div className="stat-label">Pendientes revisión</div>
          </div>
          <div className="stat-card">
            <div className="stat-value" style={{ color: 'var(--color-success)' }}>{reviewedReports}</div>
