...
function CommunitiesContent() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const ids = new Set(
        snap.docs.map((d) => d.data().communityId).filter(Boolean),
      );
  ...
  const filteredCommunities = Array.isArray(communities)
    ? communities.filter((comm) => {
        ...
        const matchesGPSFilter =
          !filterGPS || pendingGPSCommunityIds.has(comm.id);

        const commType = comm.type || "comunidad";
        const matchesTypeFilter =
          filterType === "all" || commType === filterType;
            ...
            <div className="card">
              <div className="empty-state">
                ...
                <p className="text-muted text-sm">
                  Haz clic en una comunidad para ver sus detalles, tareas y
                  asignaciones
                </p>
              </div>
            </div>
                  ...
                  >
                    ...
                    {operarios
                      .filter(
                        (o) =>
                          o.active &&
                          !assignments.find((a) => a.userId === o.uid),
                      )
                      .map((op) => (
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDocForm((f) => ({ ...f, file: e.target.files[0] }))
                    }
                    required
                  />
                </div>
