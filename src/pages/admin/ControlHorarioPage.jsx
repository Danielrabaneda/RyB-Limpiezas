...
export default function ControlHorarioPage() {
  ...
  const handleOpenEdit = (wd) => {
    const start = wd.startTime?.toDate
      ? wd.startTime.toDate()
      : new Date(wd.startTime);
    const end = wd.endTime?.toDate
      ? wd.endTime.toDate()
      : wd.status === "active"
        ? new Date()
        : new Date();
  ...
  const handleExportPDF = () => {
    if (!filters.userId) {
      alert(
        "Por favor, selecciona un operario primero para generar su informe detallado de firmas.",
      );
      return;
    }
    ...
    doc.text(
      `Período: ${format(new Date(filters.startDate), "dd/MM/yyyy")} al ${format(new Date(filters.endDate), "dd/MM/yyyy")}`,
      14,
      52,
    );
    doc.text(
    ...
    // Totals
    const totalMinutesSum = workdays.reduce(
      (acc, wd) => acc + (wd.totalMinutes || 0),
      0,
    );
    ...
    doc.text(
      `Tiempo Total Formateado: ${formatMinutes(totalMinutesSum)}`,
      14,
      finalY,
    );
    ...
    doc.text(
      "Este documento sirve como registro oficial de las jornadas realizadas en el período indicado.",
      14,
      doc.internal.pageSize.height - 10,
    );

    doc.save(
      `Control_Horario_${operarioName.replace(/\s+/g, "_")}_${filters.startDate}.pdf`,
    );
  };

  const totalMinutes = workdays.reduce(
    (acc, current) => acc + (current.totalMinutes || 0),
    0,
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-orange-800">
                ATENCIÓN: Jornadas sin cerrar
              </h4>
              <p className="text-sm text-orange-700">
                Hay {staleWorkdays.length} jornada(s) que siguen abiertas desde
                días anteriores. Los operarios verán un aviso al entrar hoy,
                pero puedes cerrarlas manualmente editando la hora de fin.
              </p>
            </div>
          </div>
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="badge badge-primary">
                    Total: {formatMinutes(op.stats.minutes)}
                  </span>
                </div>
