...
export default function QuotesPage() {
  ...
  const generatePdf = (download = true) => {
    ...
    const pdfLineRows = (globalMode) => {
      const rows = []; let currentSection = null;
      editing.lines.forEach((line) => {
        const section = line.section || "Servicio";
        if (section !== currentSection) {
          rows.push([{ content: section.toUpperCase(), colSpan: globalMode ? 2 : 7, styles: { fillColor: [232, 247, 244], textColor: [8, 123, 112], fontStyle: "bold", fontSize: 8.5, cellPadding: 2.8 } }]);
          currentSection = section;
        }
        rows.push(globalMode
          ? [`${line.concept}\n${line.description || ""}`, frequencyLabel(line.frequency)]
          : [`${line.concept}\n${line.description || ""}`, frequencyLabel(line.frequency), `${line.quantity} ${line.unit}`, money.format(line.unitPrice), `${line.discount}%`, `${line.tax}%`, money.format(line.total)]);
      });
      return rows;
    };
    if (editing.pricingMode === "global") {
      autoTable(pdf, { startY: 87, head: [["Trabajos incluidos", "Frecuencia"]], body: pdfLineRows(true), headStyles: { fillColor: accent }, styles: { fontSize: 8, cellPadding: 2.5 }, columnStyles: { 0: { cellWidth: 140 }, 1: { cellWidth: 35 } } });
      const priceStart = pdf.lastAutoTable.finalY + 9; pdf.setFontSize(11); pdf.setTextColor(30); pdf.text("PRECIO Y FORMA DE PAGO", 15, priceStart);
      autoTable(pdf, { startY: priceStart + 4, head: [["Concepto", "Cantidad", "Precio", "IVA", "Total"]], body: (editing.priceItems || []).map((item) => [`${item.concept}\n${item.description || ""}`, `${item.quantity} ${item.unit}`, money.format(item.unitPrice), `${item.tax}%`, money.format((item.total || 0) + (item.taxAmount || 0))]), headStyles: { fillColor: [46, 125, 116] }, styles: { fontSize: 7.5, cellPadding: 2.3 }, columnStyles: { 0: { cellWidth: 88 } } });
    } else {
      autoTable(pdf, { startY: 87, head: [["Concepto y descripción", "Frecuencia", "Cant.", "Precio", "Dto.", "IVA", "Total"]], body: pdfLineRows(false), headStyles: { fillColor: accent }, styles: { fontSize: 7.5, cellPadding: 2.3 }, columnStyles: { 0: { cellWidth: 67 }, 1: { cellWidth: 22 } } });
    }
