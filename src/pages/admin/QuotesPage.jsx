...
export default function QuotesPage() {
  ...
  const startQuote = async (template = TEMPLATES[0]) => {
    ...
    const data = {
      ...
      dateFormat: "dmy", headerColor: settings.brandColor || "#2563eb", companyPhone: settings.phone || "", companyEmail: settings.smtpEmail || settings.email || "", companyWebsite: settings.website || "", websiteAlignment: "center", websiteFontSize: 9,
...
function QuoteEditor({ quote, communities, settings, savedState, onChange, onClient, onLine, onLines, onPriceItems, onPricingMode, onClose, onPdf, onStatus, onEmail, onWhatsApp, onInvoice, onService }) {
  ...
  const companyEmail = quote.companyEmail ?? settings.smtpEmail ?? settings.email ?? "";
...
function PdfPreview({ quote, settings }) {
  ...
  return <div className={`paper ${global ? "paper-global" : "paper-detailed"}`} data-quote-pdf-preview>
    ...
    <div className="paper-head"><div className="paper-company">{settings.logoBase64 ? <img src={settings.logoBase64} /> : <span>LG</span>}<strong>{settings.companyName || "LimpiaGest"}</strong><small>{settings.nif || "Servicios profesionales de limpieza"}<br/>{settings.address}</small>{(companyPhone || companyEmail) && <div className="paper-company-contact">{companyPhone && <b>Teléfono para aceptar: {companyPhone}</b>}{companyEmail && <b>Email: {companyEmail}</b>}</div>}</div><div><h2>PRESUPUESTO</h2><strong>{quote.number}</strong></div></div>
