      ...
      data.documents.forEach(doc => {
        ...
        if (docUserId === userId) {
          ...
          if (colName === 'workdays' && status === 'active') {
            ...
            const hasCheckOut = fields.checkOutTime && fields.checkOutTime.nullValue === undefined;
            if (!hasCheckOut) {
              isStaleOrOpen = true;
            } else {
              checkOutTime = fields.checkOutTime.timestampValue;
            }
          } else if (colName === 'scheduledServices' && status === 'in_progress') {
            isStaleOrOpen = true;
          }
          
          // Also print if date is around May 14th, 2026 (2026-05-14)
          const dateVal = fields.date?.timestampValue || fields.checkInTime?.timestampValue || fields.startTime?.timestampValue;
          ...
          if (isStaleOrOpen || isThursdayMay14) {
            ...
            if (fields.checkOutTime) console.log(`  CheckOutTimeRaw: ${JSON.stringify(fields.checkOutTime)}`);
