...
async function handleGarageCompletion(serviceId, completionDate) {
  ...
  console.log(
    `Updated task ${serviceData.communityTaskId} startDate to: ${completionDateStr}`,
  );
  ...
  if (!nextSnap.empty) {
    ...
    if (nextServices.length > 0) {
      ...
      console.log(
        `Found next pending service: ${targetService.id} scheduled for ${format(targetDate, "yyyy-MM-dd")}`,
      );
      ...
      console.log(
        "No pending services found in database after completion date.",
      );
    }
  } else {
