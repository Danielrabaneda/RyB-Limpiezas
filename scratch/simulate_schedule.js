...
function shouldScheduleOnDay(task, date) {
  ...
  const taskCreationDateRaw = task.createdAt
  ...
  if (task.flexibleWeek) {
    ...
    if (
      ...
      (task.frequencyType === "monthly" ||
        task.frequencyType === "bimonthly" ||
        task.frequencyType === "trimonthly" ||
        task.frequencyType === "semiannual" ||
        task.frequencyType === "annual")
    ) {
      const weekNum = getWeekOfMonth(date, { weekStartsOn: 1 });
    ...
    switch (task.frequencyType) {
      case "bimonthly":
        if (currentMonthIdx % 2 !== 0) return false;
        break;
      case "trimonthly":
        if (currentMonthIdx % 3 !== 0) return false;
        break;
      case "semiannual":
        if (currentMonthIdx % 6 !== 0) return false;
        break;
      case "annual":
        if (currentMonthIdx !== 0) return false;
        break;
      case "biweekly":
        if (taskStart) {
          const weeksDiff = Math.floor(
            (date.getTime() - startOfDay(taskStart).getTime()) /
              (7 * 24 * 60 * 60 * 1000),
          );
  ...
  switch (task.frequencyType) {
    ...
    case "bimonthly":
      if (taskStart) {
        const monthDiff =
          (date.getFullYear() - taskStart.getFullYear()) * 12 +
          (date.getMonth() - taskStart.getMonth());
        if (monthDiff % 2 !== 0) return false;
      } else if (currentMonth % 2 === 0) return false;
      return task.monthDays?.length > 0
        ? task.monthDays.includes(dayOfMonth)
        : isDefaultDay();
...
const days = eachDayOfInterval({
  start: new Date("2026-04-23T00:00:00"),
  end: addDays(new Date("2026-04-23T00:00:00"), 90),
});
