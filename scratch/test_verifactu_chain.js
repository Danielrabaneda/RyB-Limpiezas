...
async function main() {
  ...
  try {
    ...
    invoicesData.forEach((inv, index) => {
      ...
      console.log(
        `  PreviousHash:  ${inv.previousHash || "(Primera de la cadena)"}`,
      );
    });
    ...
    console.log(
      `  ¿Coincide previousHash F2 con hash F1?: ${chain1Valid ? "SÍ (VÁLIDO) ✅" : "NO (ERROR) ❌"}`,
    );
    console.log(
      `  ¿Coincide previousHash F3 con hash F2?: ${chain2Valid ? "SÍ (VÁLIDO) ✅" : "NO (ERROR) ❌"}`,
    );
    ...
    try {
      ...
      console.log(
        `✅ CORRECTO: Error esperado al actualizar: "${err.message}"`,
      );
    }
