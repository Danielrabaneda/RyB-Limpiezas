  ...
  if (wdData && wdData.documents) {
    ...
    wdData.documents.forEach(doc => {
      ...
      if (docUserId === userId) {
        ...
        console.log(`[Workday] ID: ${docId}, Status: ${status}, Date: ${date}, StartTime: ${startTime}, EndTime: ${endTime}`);
      }
    });
  }
  ...
  if (ciData && ciData.documents) {
    ...
    ciData.documents.forEach(doc => {
      ...
      if (docUserId === userId) {
        ...
        console.log(`[CheckIn] ID: ${docId}, CheckInTime: ${checkInTime}, CheckOutTime: ${checkOutTime}, ServiceId: ${scheduledServiceId}`);
      }
    });
  }
  ...
  if (ssData && ssData.documents) {
    ...
    ssData.documents.forEach(doc => {
      ...
      if (docUserId === userId) {
        ...
        const status = fields.status?.stringValue;
        const date = fields.date?.timestampValue;
        console.log(`[ScheduledService] ID: ${docId}, Status: ${status}, Date: ${date}`);
      }
    });
  }
}
