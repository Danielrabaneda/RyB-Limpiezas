...
async function main() {
  ...
  console.log("Fetching scheduled services for Secretario Francisco Martínez 9...");
  const svcsSnap = await getDocs(query(collection(db, "scheduledServices"), where("communityId", "==", commId)));
  ...
  todayCheckIns.forEach(c => {
    const ciDate = c.checkInTime.toDate ? c.checkInTime.toDate() : new Date(c.checkInTime);
    const coDate = c.checkOutTime?.toDate ? c.checkOutTime.toDate().toISOString() : (c.checkOutTime ? new Date(c.checkOutTime).toISOString() : "OPEN (null)");
