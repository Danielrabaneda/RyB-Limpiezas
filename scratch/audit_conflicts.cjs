...
const content = fs.readFileSync(
  "C:/Users/Hp/.gemini/antigravity/brain/4ca128fa-bcee-4543-8493-90823563b167/.system_generated/steps/1299/output.txt",
  "utf8",
);
...
for (const [key, list] of Object.entries(groups)) {
  if (list.length > 1) {
    ...
    if (
      pendingCount > 1 ||
      (list.some((s) => s.status === "completed") && pendingCount > 0)
    ) {
      console.log(`\nConflict on ${key}:`);
      list.forEach((s) => {
        console.log(
          `  - ID: ${s.id}, Status: ${s.status}, Created: ${s.createdAt}`,
        );
      });
    }
  }
