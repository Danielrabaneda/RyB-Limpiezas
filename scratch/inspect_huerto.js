...
async function main() {
  ...
  const targetComm = comms.find((c) =>
    c.name.toLowerCase().includes("huerto de los frailes 3"),
  );
  if (!targetComm) {
  ...
  const tasksSnap = await getDocs(
    query(
      ...
      where("communityId", "==", targetComm.id),
    ),
  );
  console.log("\nTasks:");
  tasksSnap.docs.forEach((d) => {
    const data = d.data();
    console.log(
      `- Task ${d.id}: ${data.taskName}, frequency: ${data.frequency}, startDate: ${data.startDate}, recurrenceType: ${data.recurrenceType}, assignedUser: ${data.assignedUserId}, createdAt: ${data.createdAt?.toDate?.() || data.createdAt}`,
    );
  });
  ...
  services.forEach((s) => {
    ...
    const createdStr = s.createdAt?.toDate
      ? s.createdAt.toDate().toISOString()
      : "unknown";
    console.log(
      `- [${dateStr}] [${s.status}] id: ${s.id}, taskId: ${s.communityTaskId}, user: ${s.assignedUserId}, createdAt: ${createdStr}`,
    );
  });
