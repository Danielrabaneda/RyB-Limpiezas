...
async function main() {
  ...
  matchingComms.forEach((c) => {
    console.log(
      `[Community] ID: ${c.id} | Name: ${c.name} | Type: ${c.type} | Active: ${c.active}`,
    );
  });
  ...
  const matchingTasks = tasks.filter((t) => {
    ...
    return (
      commName.includes("cadiz") ||
      commName.includes("albarda") ||
      taskName.includes("cadiz") ||
      taskName.includes("albarda")
    );
  });
  ...
  matchingTasks.forEach((t) => {
    const comm = communities.find((c) => c.id === t.communityId);
    console.log(
      `[Task] ID: ${t.id} | Name: ${t.taskName} | Community: ${comm?.name || "Unknown"} (${t.communityId}) | Active: ${t.active} | Frequency: ${t.frequencyType} | isGarage: ${t.isGarage}`,
    );
  });
  ...
  matchingServices.forEach((s) => {
    ...
    console.log(
      `[Service] ID: ${s.id} | Task: ${s.taskName} | Date: ${dateStr} | Status: ${s.status} | AssignedUserId: ${s.assignedUserId} | isRollover: ${s.isRollover} | rolledOverFrom: ${s.rolledOverFrom || "None"}`,
    );
  });
