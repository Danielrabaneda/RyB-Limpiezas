...
function shouldScheduleOnDay(task, date, options = {}) {
  ...
  const explicitStart = task.startDate
    ? new Date(
  ...
  const taskCreationDate = startOfDay(
    new Date(task.createdAt || "2026-05-02T19:22:45Z"),
  );
...
months.forEach((mIdx) => {
  ...
  const hasForecast = days.some((day) =>
    shouldScheduleOnDay(task, day, { isForecasting: true }),
  );
  console.log(`Month ${mIdx + 1}: ${hasForecast ? "YES" : "NO"}`);
});
