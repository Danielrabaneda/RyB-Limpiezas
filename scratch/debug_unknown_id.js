...
async function findUnknownCommunity() {
  ...
  console.log(
    "Unknown Community IDs found in checkIns:",
    Array.from(unknownIds),
  );

  for (const id of unknownIds) {
    const records = checkInsSnap.docs.filter(
      (d) => d.data().communityId === id,
    );
    console.log(`ID: ${id}`);
    console.log(`Records count: ${records.length}`);
    const sample = records[0].data();
    console.log(
      `Sample record (User: ${sample.userName}, Date: ${sample.checkInTime?.toDate().toLocaleDateString()})`,
    );
  }
}
