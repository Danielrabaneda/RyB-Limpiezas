...
async function main() {
  ...
  workdaysSnap.forEach((w) => {
    const data = w.data();
    console.log(
      `- ID: ${w.id}, User: ${data.userName || "N/A"} (UID: ${data.userId}), StartTime: ${data.startTime?.toDate()?.toISOString() || "N/A"}`,
    );
  });
  ...
  for (const c of checkInsSnap.docs) {
    ...
    console.log(
      `  InTime: ${data.checkInTime?.toDate()?.toISOString() || "N/A"}`,
    );
  }
