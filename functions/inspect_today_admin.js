...
async function main() {
  ...
  users.forEach(u => {
    console.log(`- User: ${u.name} (Email: ${u.email || u.id}, UID: ${u.id || u.uid})`);
  });
  ...
  todayServices.forEach(s => {
    ...
    console.log(`  Community: ${s.communityName || 'N/A'} (ID: ${s.communityId})`);
    console.log(`  Task: ${s.taskName || 'N/A'}`);
    console.log(`  Status: ${s.status}`);
    console.log(`  Assigned: ${s.assignedUserName || 'N/A'} (UID: ${s.assignedUserId})`);
  ...
  todayCheckIns.forEach(c => {
    const checkinDate = c.checkInTime?.toDate ? c.checkInTime.toDate().toISOString() : new Date(c.checkInTime).toISOString();
    const checkoutDate = c.checkOutTime?.toDate ? c.checkOutTime.toDate().toISOString() : "OPEN (null)";
  ...
  todayWorkdays.forEach(w => {
    const date = w.date.toDate ? w.date.toDate().toISOString().split('T')[0] : new Date(w.date).toISOString().split('T')[0];
    console.log(`- Workday ID: ${w.id}`);
    console.log(`  User: ${w.userId}`);
    console.log(`  Date: ${date}`);
    console.log(`  Start: ${w.startTime?.toDate ? w.startTime.toDate().toISOString() : 'N/A'}`);
    console.log(`  End: ${w.endTime?.toDate ? w.endTime.toDate().toISOString() : 'N/A'}`);
