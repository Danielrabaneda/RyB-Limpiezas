...
async function main() {
  ...
  for (const s of missedServices) {
    const cName = (s.communityName || '').toLowerCase();
    
    // Si es una de las 3 comunidades afectadas por el operario Daniel Rabaneda
    if (cName.includes('santa ana 3') || cName.includes('casino') || cName.includes('correos')) {
      console.log(`Updating service: ${s.id} | ${cName} | date: ${s.scheduledDate ? new Date(s.scheduledDate.toDate ? s.scheduledDate.toDate() : s.scheduledDate).toLocaleString() : 'No date'}`);
